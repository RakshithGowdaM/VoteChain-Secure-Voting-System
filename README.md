...
### If Netlify Shows "Not Working"

If your frontend opens but login/OTP/voting fail, it usually means only frontend is deployed.

- Frontend up: `https://your-site.netlify.app` returns `200`
- Backend missing: `https://your-site.netlify.app/api/health` returns `404`

Use the included backend deployment blueprint:

1. Deploy backend using [render.yaml](render.yaml) on Render.
2. After deploy, verify backend health: `https://your-render-domain/api/health`.
3. In Netlify environment variables set:
	- `REACT_APP_API_URL=https://your-render-domain/api`
4. Redeploy Netlify.
5. Ensure backend env has:
